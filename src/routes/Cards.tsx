...
const Cards = () => {
    ...
    return (
        <div className="flex flex-col">
            <div className=" bg-blue-400 m-2 dark:bg-blue-900 border rounded-md search-place pl-6">
                <input
                    type="text"
            ...
            <div className="flex flex-wrap justify-center items-center gap-5 mt-2 dark:text-white">
                ...
                {error && <div>{error}</div>}
                {filteredCards.map((c) => (
                    <div key={c._id} className="flex flex-col justify-center items-center p-5 rounded-md text-center bg-blue-400 m-2 dark:md:bg-blue-900 relative drop-shadow-md cards-style">
                        <Link to={`/cards/${c._id}`} className="flex flex-col items-center ">
                            ...
                            />
                            <div className="m-1 ">
                                <h2 style={{ color: "#000000" }} className="text-l px-1 py-0.5 font-bold">{c.title}</h2>
                        ...
                        {isAdmin &&
                            <button
                                className={`absolute bottom-10 right-2  mr-2 p-1 rounded-md text-sm bg-cyan-200 m-2 dark:md:bg-blue-900 font-medium ${'text-blue-400 dark:text-blue-900'}`}
                        ...
                        <button
                            className={`absolute bottom-2 right-2  mr-2 p-1 rounded-md text-sm bg-gray-300 dark:bg-gray-700 font-medium ${favorites.includes(c._id) ? 'text-blue-400 dark:text-blue-600' : 'text-gray-600 dark:text-gray-300'}`}
                        ...
                        <div className={`absolute bottom-2 left-2  mr-2 p-1 px-4  rounded-md text-sm bg-gray-600 dark:bg-gray-400 ${likedCards.includes(c._id) ? 'text-blue-400 dark:text-blue-600' : 'text-gray-300 dark:text-gray-600'}`}>
